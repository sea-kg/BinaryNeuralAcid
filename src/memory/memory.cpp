...
void RHMemory::load(QString filename){
    m_vItems.clear();
    QFile file(filename);
    if (!file.exists()) {
...
void RHMemory::save(QString filename){
    QFile file(filename);
    if (file.exists()) {
        file.remove();
    }
